...
\begin{impl}[Sum-formula with Haskell]
...
For example the fraction $\frac{5}{3}$ could be represented as $1.667$ with one integer and 3 decimal places, rounded to a precision of 4 digits overall.
...
\end{equation}
where $Z \in \mathbb{Q}$, $B \in \mathbb{N}$, and $Z$ has $n$ iteger and $m$ decimal places.
